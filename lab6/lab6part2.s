...
CONFIG_TIMER:     LDR      R0,=0xFF202000 
                  LDR      R1,=0x7840  // 25 * 10^6  lower 16 bits
                  STR      R1,[R0,#8]
                  LDR      R1,=0x17D    // higher 16 bits
		  STR      R1,[R0,#0xC]
                  MOV      R1,#0b0111   // start the timer and auto reset, enable interupt 
                  STR      R1,[R0,#4]
                  MOV	   PC, LR                 
...
SERVICE_UND:    SUBS     PC,LR,#4
SERVICE_SVC:	SUBS     PC,LR,#4
SERVICE_ABT_INST:	SUBS     PC,LR,#4
SERVICE_ABT_DATA:	SUBS     PC,LR,#4
SERVICE_FIQ:	SUBS     PC,LR,#4
...
TIMER_HANDLER:  
		CMP      R5, #72 // check if ID is correct
                BNE      UNEXPECTED
                BL       TIMER_ISR
                B        EXIT_IRQ
...
KEY_ISR:       PUSH      {R0-R10}
	       LDR       R4,=0xFF200050
	       LDR       R5,[R4,#0xC]
               CMP       R5,#0 //as long as one of the keys are pressed, go into SET 
               BNE       SET
	
ERROR:         B         ERROR 
              	
SET:	       STR	 R5,[R4,#0xC] //clear edge bits
	       LDR       R4,RUN
               CMP	 R4,#0 //check if RUN is 0
               BEQ	 SET_ONE
               BNE       SET_ZERO
...
TIMER_ISR:    PUSH     {R0-R7}
	      MOV      R1,#0
              LDR      R0,=0xFF202000 //timer address
              STR      R1,[R0]
	      LDR      R4,RUN
              LDR      R3,COUNT
              ADD      R3,R4 // increment count
              STR      R3,COUNT
              POP      {R0-R7}
              MOV      PC, LR
